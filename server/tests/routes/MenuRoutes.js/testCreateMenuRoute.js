...
describe('POST /api/v1/menu', () => {
  ...
  it('should create menu for today if date is not given', async () => {
    const res = await chai.request(App)
      .post('/api/v1/menu')
  ...
  it('should return a conflict status if menu is already set for a day', async () => {
    const res = await chai.request(App)
      .post('/api/v1/menu')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({
  ...
  it('should not be able to create for a past date', async () => {
    const res = await chai.request(App)
      .post('/api/v1/menu')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({
